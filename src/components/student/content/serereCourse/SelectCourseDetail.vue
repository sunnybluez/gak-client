    ...
    export default {
                ...
                return h('div', [
                  h('Button', {
                    ...
                    on: {
                      click: () => {
                        // console.log(event);
                        this.selectCourse(params.index)
                      }
                    }
                  }, '选课')
                ]);
      ...
      methods: {
        selectCourse(index){
          ...
          selectCourseHttp(studentId,releaseCourseId).then((data)=>{
            this.$Message.success(this.data[index].name+"成功选课 等待结果")
      ...
      mounted() {
        ...
        let studentId = this.$store.getters.getUser.id;
        getAllCanSelectCourseHttp(studentId,term).then((data)=>{
          this.data = data;
        }).catch((err)=>{
